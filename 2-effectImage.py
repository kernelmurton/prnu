...
def main(device: str):
    ...
    dict = {}
    # 画像加工の手法
    e_methods = ['blur', 'GaussianBlur', 'medianBlur',
                 'GammaCorrection', 'morphologyOP', 'morphologyCL']
    # 画像加工の手法ごとにフォルダを作成
    for method in e_methods:
        if method == 'GaussianBlur':
            for sizeNum in sizeNums:
                for i in range(3):
                    path = effectedDirpath + '/' + method + \
        ...
        for method in e_methods:
            if method == 'GaussianBlur':
                ...
                for sizeNum in sizeNums:
                    k_size = (sizeNum, sizeNum)
                    dst_key = method+str(sizeNum)
                    dst_value = cv2.morphologyEx(
                        gray_3ch, cv2.MORPH_OPEN, kernel=k_size)
                    dict[dst_key] = dst_value
                ...
                for sizeNum in sizeNums:
                    ...
                    dst_key = method+str(sizeNum)
                    dst_value = cv2.morphologyEx(
                        gray_3ch, cv2.MORPH_CLOSE, kernel=k_size)
                    dict[dst_key] = dst_value

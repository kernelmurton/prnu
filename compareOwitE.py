...
def main(device_name: str, method: str):
...
if __name__ == '__main__':
    ...
    savepath = 'e-Date/Effected'
    files = sorted(os.listdir(editpath))
    device_list = [f for f in files if os.path.isdir(
        os.path.join(editpath, f))]
    for device_name in device_list:
        savepath = savepath + '/' + device_name
        method_list = [f for f in files if os.path.isdir(
            os.path.join(savepath, f))]
        for method in method_list:
            main(device_name=device_name, method=method)
